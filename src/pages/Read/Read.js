...
const Read = ({}) => {
  ...
  useEffect(()=>{
    setLoading(prevState => {
        return {...prevState, status:true,message:"Please wait, fetching data..."};
      });
  ...
  useEffect(() => {
    setAudioFile(
        new Audio(storyData?.story[page-1]?.audio)
      );
  }, [page]);
  ...
  useEffect(() => {
    if (storyData && audioFile && Object.keys(storyData).length !== 0 && page!==0) {
          ...
          <div className={classes.contentFlex}>
            ...
            <div className={classes.content}>
              {page === 0 ? (
                ...
                <StoryRead data={storyData?.story[page - 1]} 
